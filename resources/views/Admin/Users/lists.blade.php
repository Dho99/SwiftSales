        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="changeToRole">Edit</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="changeRole" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" id="editRoleBtn">Simpan</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function findDatas() {
            findData(currUrl).then(function(response) {
                reinitializeTable('#customersTable', response.users, [{
                        data: null,
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row){
                            return `<div class="badge ${row.roles == 'Admin' ? 'bg-dark' : row.roles == 'Cashier' ? 'bg-secondary' : 'bg-primary'}">${row.roles}</div>`
                        }
                    },
                    {
                        data: null,
        ...
        function deleteCustomer(id){
            const url = '/user/'+id;
            swalConfirm('Apakah anda yakin akan menghapus data Pelanggan ?').then(function(result){
                if(result){
